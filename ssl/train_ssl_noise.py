...
def train_and_evaluate(cfg):
    ...
    #Training settings
    experiment_dir = os.path.join('experiments',cfg.exp_type,cfg.save_dir,cfg.pretext)
    # Foi adicionado cfg.pretext para que seja possível diferenciar noise de blur
    ...
    #if cfg.pretext == 'noise':
    #    images, noise_labels = next(iter(dloader_train))  # Para a tarefa de "noise"

    #elif cfg.pretext == 'relative_position':
...
#    print('\nEvaluate on test')
    best_ckpt_path = os.path.join(experiment_dir, '{}_best.pth'.format(cfg.network.lower()))
    logging.info('\nEvaluate test result on best ckpt: {}'.format(best_ckpt_path))
    ...
    utils.save_yaml(cfg,save_path=os.path.join(experiment_dir,'config_ssl_relpos.yaml'))
    logging.info('-----------End of Experiment------------')

if __name__=='__main__':
    config_file='config/config_ssl_noise.yaml'
